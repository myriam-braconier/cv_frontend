...
const ListSynthetisers: React.FC<ListSynthetisersProps> = ({ synths, userRoles }) => {
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [localSynths, setLocalSynths] = useState<Synth[]>(synths);
  ...
  const fetchSynths = useCallback(async () => {
    try {
      ...
      setLocalUserRoles(userRole === "admin" ? ["admin"] : [userRole]);
      setLocalSynths(synthResponse.data.data);
    } catch (error) {
