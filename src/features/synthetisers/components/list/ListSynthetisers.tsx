...
export default function ListSynthetisers() {
    ...
    if (isLoading) {
        return <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500" />;
    }
    ...
    if (error) {
        return (
            <div className="text-red-500">
                {error}
                <button onClick={fetchSynths} className="bg-blue-500 text-white px-4 py-2 rounded">
                    Réessayer
                </button>
            </div>
        );
